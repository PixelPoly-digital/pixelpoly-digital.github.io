...
PIXELPOLY.core={functions:function(){PIXELPOLY.core.scrollTop()
PIXELPOLY.core.rtlStatus()
PIXELPOLY.core.equalize()
PIXELPOLY.core.customHeight()
PIXELPOLY.core.darkTheme()},scrollTop:function(){var $scrollTop=$("#scrollTop")
...
$darkBtnTrigger.on("click",function(e){darkElemSrc();$body.addClass("dark");PIXELPOLY.elements.shapeDivider();Cookies.set(darkColorScheme,!0,{expires:Number(365)})})
$lightBtnTrigger.on("click",function(e){lightElemSrc();$body.removeClass("dark");PIXELPOLY.elements.shapeDivider();Cookies.remove(darkColorScheme)})
...
elem.addClass("toggle-active");$body.addClass("mainMenu-open");PIXELPOLY.header.logoStatus();$mainMenu.animate({"min-height":$window.height()},{duration:500,easing:"easeInOutQuart",start:function(){setTimeout(function(){$mainMenu.addClass("menu-animate")},300)},complete:function(){processing=!1}})}}
var closeMenu=function(){if(!processing){processing=!0;Settings.menuIsOpen=!1;PIXELPOLY.header.logoStatus();$mainMenu.animate({"min-height":0},{start:function(){$mainMenu.removeClass("menu-animate")},done:function(){$body.removeClass("mainMenu-open");elem.removeClass("toggle-active");if(Settings.submenuLight&&Settings.headerHasDarkClass&&Settings.headerDarkClassRemoved&&!$header.hasClass("header-sticky")){$header.addClass("dark")}
...
return elemCurrent}}
PIXELPOLY.slider={functions:function(){PIXELPOLY.slider.PIXELPOLYSlider();PIXELPOLY.slider.carousel()},PIXELPOLYSlider:function(){if($PIXELPOLYSlider.length>0){if(typeof $.fn.flickity==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery flickity slider plugin is missing in plugins.js file.","danger");return!0}
...
getCarouselColumns();var itemWidth;elem.find("> *").wrap('<div class="polo-carousel-item">');if(elem.hasClass("custom-height")){elem.options.setGallerySize=!1;PIXELPOLY.core.customHeight(elem);PIXELPOLY.core.customHeight(elem.find(".polo-carousel-item"));var carouselCustomHeightStatus=!0}
...
if(!$.isNumeric(elem.options.autoPlay)&&elem.options.autoPlay!=!1){elem.options.autoPlay=Number(7000)}
var $carouselElem=$(elem);$carouselElem.imagesLoaded(function(){$carouselElem.flickity({cellSelector:elem.options.cellSelector,prevNextButtons:elem.options.prevNextButtons,pageDots:elem.options.pageDots,fade:elem.options.fade,draggable:elem.options.draggable,freeScroll:elem.options.freeScroll,wrapAround:elem.options.wrapAround,groupCells:elem.options.groupCells,autoPlay:Number(elem.options.autoPlay),pauseAutoPlayOnHover:elem.options.pauseAutoPlayOnHover,adaptiveHeight:elem.options.adaptiveHeight,asNavFor:elem.options.asNavFor,initialIndex:elem.options.initialIndex,accessibility:elem.options.accessibility,setGallerySize:elem.options.setGallerySize,resize:elem.options.resize,cellAlign:elem.options.cellAlign,rightToLeft:elem.options.rightToLeft,contain:!0});elem.addClass("carousel-loaded")});if(elem.hasClass("custom-height")){PIXELPOLY.core.customHeight(elem)}
if(Number(elem.options.items)!==1){$(window).on("resize",function(){setTimeout(function(){getCarouselColumns();itemWidth=(elem.width()+Number(elem.options.margin))/setResponsiveColumns;if(elem.options.autoWidth||carouselCustomHeightStatus){elem.find(".polo-carousel-item").css({"padding-right":elem.options.margin+"px"})}else{if(!elem.hasClass("custom-height")){elem.find(".polo-carousel-item").css({"width":itemWidth,"padding-right":elem.options.margin+"px"})}else{PIXELPOLY.core.customHeight(elem.find(".polo-carousel-item"));elem.find(".polo-carousel-item").css({"width":itemWidth,"padding-right":elem.options.margin+"px"})}}
elem.find(".flickity-slider").css({"margin-right":-elem.options.margin/setResponsiveColumns+"px"})
elem.flickity("reposition")},100)})}})}}}
PIXELPOLY.elements={functions:function(){PIXELPOLY.elements.shapeDivider();PIXELPOLY.elements.naTo();PIXELPOLY.elements.morphext();PIXELPOLY.elements.buttons();PIXELPOLY.elements.accordion();PIXELPOLY.elements.animations();PIXELPOLY.elements.parallax();PIXELPOLY.elements.backgroundImage();PIXELPOLY.elements.responsiveVideos();PIXELPOLY.elements.countdownTimer();PIXELPOLY.elements.progressBar();PIXELPOLY.elements.pieChart();PIXELPOLY.elements.maps();PIXELPOLY.elements.gridLayout();PIXELPOLY.elements.tooltip();PIXELPOLY.elements.popover();PIXELPOLY.elements.magnificPopup();PIXELPOLY.elements.yTPlayer();PIXELPOLY.elements.vimeoPlayer();PIXELPOLY.elements.modal();PIXELPOLY.elements.sidebarFixed();PIXELPOLY.elements.clipboard();PIXELPOLY.elements.bootstrapSwitch();PIXELPOLY.elements.countdown();PIXELPOLY.elements.other();PIXELPOLY.elements.videoBackground();PIXELPOLY.elements.forms();PIXELPOLY.elements.formValidation();PIXELPOLY.elements.formAjaxProcessing();PIXELPOLY.elements.floatingDiv();PIXELPOLY.elements.wizard();PIXELPOLY.elements.counters()},forms:function(){var $showHidePassword=$(".show-hide-password")
...
if($textRotator.length>0){if(typeof $.fn.Morphext==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery Morphext plugin is missing in plugins.js file.","danger")
return!0}
...
if($animate.length>0){if(typeof Waypoint==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery Waypoint plugin is missing in plugins.js file.","danger")
return!0}
...
if($parallax.length>0){if(typeof $.fn.scrolly==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery scrolly plugin is missing in plugins.js file.","danger")
return!0}
...
if($counter.length>0){if(typeof $.fn.countTo==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery countTo plugin is missing in plugins.js file.","danger")
return!0}
...
if($countdownTimer.length>0){if(typeof $.fn.countdown==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery countdown plugin is missing in plugins.js file.","danger")
return!0}
...
if($pieChart.length>0){if(typeof $.fn.easyPieChart==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery easyPieChart plugin is missing in plugins.js file.","danger")
return!0}
...
if($map.length>0){if(typeof $.fn.gmap3==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery gmap3 plugin is missing, please go to this <a href='//support.PIXELPOLYthemes.com/help-center/articles/8/14/65/google-maps'>help article</a> and follow instructions on how to configure google maps.","danger")
return!0}
...
marker.addListener("click",function(){infowindow.open(map)})})}})}},gridLayout:function(){if($gridLayout.length>0){if(typeof $.fn.isotope==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery isotope plugin is missing in plugins.js file.","danger")
return!0}
var isotopeRTL;if(PIXELPOLY.core.rtlStatus()==!0){isotopeRTL=!1}else{isotopeRTL=!0}
...
if(elem.next().hasClass("infinite-scroll")){PIXELPOLY.elements.gridLayoutInfinite(elem,elem.options.itemSelector,elem.options.gridMargin)}
...
$filterItem.on("click",function(){elemFilter.find("li").removeClass($filterItemActiveClass);$(this).parent("li").addClass($filterItemActiveClass);var filterValue=$(this).attr("data-category");$(elemFilterLayout).isotope({filter:filterValue}).on("layoutComplete",function(){$window.trigger("scroll");PIXELPOLY.elements.naTo()})
...
filterDefaultValue.parent("li").addClass($filterItemActiveClass)}})}})}},gridLayoutInfinite:function(element,elementSelector,elemGridMargin){if(typeof $.fn.infiniteScroll==='undefined'){PIXELPOLY.elements.notification("Warning","jQuery infiniteScroll plugin is missing, please add this code line <b> &lt;script src=&quot;plugins/metafizzy/infinite-scroll.min.js&quot;&gt;&lt;/script&gt;</b>, before <b>plugins.js</b>","danger");return!0}
...
elem.on('append.infiniteScroll',function(event,response,path,items){PIXELPOLY.slider.carousel($(items).find('.carousel'));loadMoreBtn.html(loadMoreBtnText);element.css("margin","0 -"+gridMargin+"px -"+gridMargin+"px 0");element.find('.'+gridItem).css("padding","0 "+gridMargin+"px "+gridMargin+"px 0")})},tooltip:function(){var $tooltip=$('[data-toggle="tooltip"]')
if($tooltip.length>0){if(typeof $.fn.tooltip==="undefined"){PIXELPOLY.elements.notification("Warning: jQuery tooltip plugin is missing in plugins.js file.","warning")
return!0}
$tooltip.tooltip()}},popover:function(){var $popover=$('[data-toggle="popover"]')
if($popover.length>0){if(typeof $.fn.popover==="undefined"){PIXELPOLY.elements.notification("Warning: jQuery popover plugin is missing in plugins.js file.","warning")
return!0}
$popover.popover({container:"body",html:!0})}},magnificPopup:function(){var $lightbox=$("[data-lightbox]")
if($lightbox.length>0){if(typeof $.fn.magnificPopup==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery magnificPopup plugin is missing in plugins.js file.","danger")
return!0}
...
this.st.mainClass="mfp-zoom-out"}}},ajax:{type:"ajax",removalDelay:500,callbacks:{ajaxContentAdded:function(mfpResponse){PIXELPOLY.slider.carouselAjax()
PIXELPOLY.elements.responsiveVideos()
PIXELPOLY.elements.buttons()}}},inline:{type:"inline",removalDelay:500,callbacks:{beforeOpen:function(){this.st.image.markup=this.st.image.markup.replace("mfp-figure","mfp-figure mfp-with-anim")
...
if($ytPlayer.length>0){if(typeof $.fn.YTPlayer==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery YTPlayer plugin is missing, please add this code line <b> &lt;script src=&quot;plugins/youtube-player/jquery.mb.YTPlayer.min.js&quot;&gt;&lt;/script&gt;</b>, before <b><--Template functions--></b>","danger",10000)
return!0}
...
if($vmPlayer.length>0){if(typeof $.fn.vimeo_player==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery vimeo_player plugin is missing, please add this code line <b> &lt;script src=&quot;plugins/vimeo-player/jquery.mb.vimeo_player.min.js&quot;&gt;&lt;/script&gt;</b>, before <b><--Template functions--></b>","danger",10000)
return!0}
...
if(!$message){$message=""}
$element="body"
var notify=function(){$.notify({title:$title,message:$message},{element:$element,type:$type||"warning",delay:$delay||10000,template:'<div data-notify="container" '+$backgroundImageContainer+' class="bootstrap-notify '+$elementContainer+' alert alert-{0}" role="alert">'+'<button type="button" aria-hidden="true" class="close" data-notify="dismiss">Ã—</button>'+'<span data-notify="icon"></span> '+'<span data-notify="title">{1}</span> '+'<span data-notify="message">{2}</span>'+"</div>",mouse_over:!0,allow_dismiss:!0,placement:{from:$placement},animate:{enter:"animated "+$animateEnter,exit:"animated "+$animateExit}})}
if($timeout){setTimeout(function(){notify()},2000)}else{notify()}},sidebarFixed:function(){if(PIXELPOLY.core.rtlStatus()){return!0}
var $sidebarFixed=$(".sticky-sidebar")
if($sidebarFixed.length>0){if(typeof $.fn.theiaStickySidebar==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery theiaStickySidebar plugin is missing in plugins.js file.","danger")
return!0}
...
if($bootstrapSwitch.length>0){if(typeof $.fn.bootstrapSwitch==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery bootstrapSwitch plugin is missing in plugins.js file.","danger")
return!0}
...
if($clipboardTarget.length>0){if(typeof ClipboardJS==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery ClipboardJS plugin is missing in plugins.js file.","danger")
return!0}
...
PIXELPOLY.widgets={functions:function(){PIXELPOLY.widgets.twitter()
PIXELPOLY.widgets.flickr()
PIXELPOLY.widgets.instagram()
PIXELPOLY.widgets.subscribeForm()},twitter:function(){var $widget_twitter=$(".widget-tweeter")||$(".widget-twitter")
if($widget_twitter.length>0){if(typeof $.fn.twittie==="undefined"){PIXELPOLY.elements.notification("Warning","jQuery twittie plugin is missing in plugins.js file.","danger")
return!0}
...
$(document).ready(function(){PIXELPOLY.core.functions();PIXELPOLY.header.functions();PIXELPOLY.slider.functions();PIXELPOLY.widgets.functions();PIXELPOLY.elements.functions()})
$window.on("scroll",function(){PIXELPOLY.header.stickyHeader();PIXELPOLY.core.scrollTop();PIXELPOLY.header.dotsMenu()})
$window.on("resize",function(){PIXELPOLY.header.logoStatus();PIXELPOLY.header.stickyHeader()})})
(jQuery)